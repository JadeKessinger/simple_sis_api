...
def getSitesNearSite(se, netcode, lookupcode, isactive="yes"):
    ...
    if numsites > 1:
        logger.warning(
            f"Expecting one site but fetched {numsites}. Using first site from list of sites {sites[0]}"
        )
    ...
    bbkw = dict(
        latitude_lte=lat + 0.5,
        latitude_gte=lat - 0.5,
        longitude_lte=lon + 0.5,
        longitude_gte=lon - 0.5,
    )
...
def main():
    examples = dict(
        s1="CO Sites Active as of 2017-09-21",
    ...
    parser = argparse.ArgumentParser(
        description="Wrapper for running various examples for simple_sis_api"
    )
    parser.add_argument("inifile", help="Path and name of ini file")
    parser.add_argument(
        "mode", choices=["test", "prod"], help="Connect to sis test or prod site"
    )
    parser.add_argument(
        "example",
    ...
    try:
        ...
        if options.example == "s1":
            ...
            equipinstalls = currentlyInstalledEquipAtNet(
                ei, netcode="CI", category="LOGGER", modelname="Q8"
            )
        elif options.example == "e2":
            equipinstalls = currentlyInstalledEquipAtNet(
                ei, netcode="CI", lookupcode="WWF"
            )
        ...
        for s in sites:
            print(
                f"    {s['netcode']} {s['lookupcode']} ({s['ondate']} - {s['offdate']})"
            )

        print(f"  Number of equipment installations: {len(equipinstalls)}")
        for eq in equipinstalls:
            print(
                f"    {eq['category']} {eq['modelname']} s/n {eq['serialnumber']} installed at {eq['netcode']} {eq['lookupcode']} on {eq['ondate']}"
            )
