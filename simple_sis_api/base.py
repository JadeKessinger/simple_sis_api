...
class APIBase(object):
    ...
    def get_filtered_list(self, filterby, sortby=[]):
        ...
        for k, v in filterby.items():
            if k in self.allowed_multivalue_filters:
                ...
                self.logger.warning(
                    f'Filter param "{k}" not supported by endpoint {self.endpointurl}'
                )
        ...
        filtered_data = [
            elem for elem in elem_list if self._filter_data(elem, client_filters)
        ]
    ...
    def _flatten_data(self, data, lookup={}):
        ...
        elem_list = []
        for elem in data:
            elem_detail = {"type": elem["type"], "id": int(elem["id"])}
            ...
            for attr, val in elem["attributes"].items():
                if attr in ssa.ATTR_DATATYPE_MAPPING and val is not None:
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Unable to cast {attr} value {val} to {ssa.ATTR_DATATYPE_MAPPING[attr]}. Error: {e}"
                        )
